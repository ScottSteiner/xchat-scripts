...
my $NAME    = 'steiner:terror';
my $VERSION = '1.0';

register($NAME, $VERSION, "Terror alias rewrite by Scott Steiner and others");

IRC::add_command_handler("terror", "terror");

sub terror {
	my $sentence = sent_struct($_[0]);
...
my @people_politicians_US_other = (
	"Alan Greenspan",
	"Ben Bernanke",
	"Cindy Shehan",
	"Colin Powell",
...
my @people_politicians_US_supremecourt = (
...
my @people_politicians_US_presidents = (
...
my @people_politicians_US_senators = (
...
my @locations_US_military_AirForceBases = (
...
my @locations_US_military_NavalShips = (
...
my @conjugations = ('also','although it seems','and','because','but','for purposes of','furthermore','however','inconclusive','like','next','or','then','therefore','thus','whereas','with');
...
# Cells for sentence structure 43 -- Idea by and implimented by thinkpol, thanks!
my @celln = ( "One", "Two", "Three", "Four", "Five", "Six", "Seven", "Eight", "Niner", "Zero" );
...
# Currencies (made up) @money and @barter_items will be added to this list
my @currencies = (
	"American Express",
	"AMEX blue sky miles",
...
sub sent_struct {
	...
	push(@currencies,@barter_items,@money); 
	push(@locations,@locations_US_cities,@locations_US_military_AirForceBases,@locations_US_military_NavalShips,@locations_US_states,@locations_World_cities,@locations_World_countries);
	push(@people,@people_entertainers,@people_fiction,@people_islam,@people_military,@people_politicians_US_other,@people_politicians_US_presidents,@people_politicians_US_senators,@people_politicians_World,@people_technology,@people_terrorists,@people_titles);
	push(@weapons_scifi,@weapons_Babylon5,@weapons_Farscape,@weapons_Stargate,@weapons_StarTrek,@weapons_StarWars);
	push(@weapons,@weapons_scifi);
	my $barter_items	= $barter_items[ int rand @barter_items ];
	my $money			= $money[ int rand @money ];
	my $ending			= $endings[ int rand @endings ];
	my $cellnum			= $cell[ int rand @cell ] . ' ' . $cell[ int rand @cell ] . ' ' . $cell[ int rand @cell ] . ' ' . $celln[ int rand @celln ] . ' ' . $celln[ int rand @celln ];
	my $personarg		= $_[0]; #Gets the name from the command line argument.  PROTIP: Use quotes
	my $person1; 
	if ($personarg) { $personarg =~ s/\s+$//; $person1 = $personarg;} else { $person1 = $people[ int rand @people ]; }
	my $person2			= $people[ int rand @people ];
	my $currency1		= $currencies[ int rand @currencies ];
	my $currency2		= $currencies[ int rand @currencies ];
	my $location1		= $locations[ int rand @locations ];
	my $location2		= $locations[ int rand @locations ];
	my $conj1			= $conjugations[ int rand @conjugations ];
	my $conj2			= $conjugations[ int rand @conjugations ];
	my $weapon1			= $weapons[ int rand @weapons ];
	my $weapon2			= $weapons[ int rand @weapons ];
	my $thing1			= $thing[ int rand @thing ];
	my $thing2			= $thing[ int rand @thing ];
	my $verbpresent1	= $verbpresent[ int rand @verbpresent ];
	my $verbpresent2	= $verbpresent[ int rand @verbpresent ];
	my $verbpast1		= $verbpast[ int rand @verbpast ];
	my $verbpast2		= $verbpast[ int rand @verbpast ];
	my $pnoun1			= $pnoun[ int rand @pnoun ];
	my $pnoun2			= $pnoun[ int rand @pnoun ];
	my $begin1			= $begin[ int rand @begin ];

	#Change this number to the number of sentences minus 1
	my $Ts = int(rand 48);
	if ( $Ts == 0 )  { return "God be with you. Have you been followed? Did you carry out your attack against $person1? ... Damn! Move against $person2 at once, with $weapon1 or $weapon2. Good luck, in the name of Allah!"; }
	if ( $Ts == 1 )  { return "Silence, I bring news of the glory of Allah from $person1. We are going to bring the wrath of Mohamhed down upon $location1 using $weapon1 and $weapon2. We will rendezvous at $location2 in two weeks to plan the attack. Peace be with you."; }
	if ( $Ts == 2 )	 { return "Hi. I heard on Al Jazeera that $location1 was attacked! Has it made CNN yet?? Rumors are that it was attacked with $weapon1. The source of this news is reported as $person1, who supposedly heard it from $person2."; }
	if ( $Ts == 3 )	 { return "AP, $location1 -- In a stunning move of defiance, $person1 is in place to attack $location1 with $weapon1. Due to the severity of such an accusation, we have confirmed this with $person2. They also claim that they have reserves of $weapon2 ready to go. This attack may be stopped, it is claimed, if a ransom of $currency1 is paid."; }
	if ( $Ts == 4 )	 { return "Al Jazeera, $location1 -- The infidel-backed $person1, with additional backing from other western groups, has kidnapped $person2. This has prompted a threat of Jihad using $weapon1 upon $location1 by the Al Aqsa Martyrs brigade. More news as we get it. Praise Allah!"; }
	if ( $Ts == 5 )	 { return "$person1, who was found smuggling $weapon1 into $location1, was caught today by $person2. This is a severe set back in their plans, as they had planned to next attack $location2 with $weapon2, unless they were paid quickly with $currency1."; }
	if ( $Ts == 6 )	 { return "Damn. I just got a call from my dad, have you guys heard about this shit? There was some kinda attack on $location1 :( This sucks... I was going to school but $person1 reports the attack was done with $weapon1, isn't this awful?! Has it hit Fox News yet?"; }
	if ( $Ts == 7 )	 { return "Myself and my Band of Brothers are looking for a few items. Perhaps you can help us. We have large ammounts of $currency1 we can pay with... 1) $weapon1 2) $weapon2 ... This would greatly assist our Holy War! Please contact $person1 if you are interested."; }
	if ( $Ts == 9 )	 { return "This is for AT&T and the NSA: You can kiss my ass, I'm not paying my next bill. Instead I will kidnap $person1 and then threaten to destroy $location1 with $weapon1, you mother fuckers! Time for you bitches to pay up. In $currency1. Fuck you" . $ending . ""; }
	if ( $Ts == 10 ) { return "As an American born citizen, I am sick and tired of my governments bullshit! We have NO BUSINESS interfering in other countries affairs.. You wanted action? Okay you got it! I have $weapon1 and I'm not afraid to use it... I live in $location1, and I'm not afraid to do it here, who gives a shit if I die. I don't care what $person1 says. 14/88!"; }
	if ( $Ts == 11 ) { return "For Sale: High definition videos of $person1 $verbpresent1 $person2... We realize that this is difficult to beleive, but for a small ammount of $currency1 we will let you preview it for yourself!"; }
	if ( $Ts == 12 ) { return "I have already spoken to you about this, but I am aware of a source at $location1 who can provide large ammounts of $weapon1. I am not exaclty sure who it is, but $person1 is the liason. We really need $money so that is what we will accept."; }
	if ( $Ts == 13 ) { return "Wanted: $weapon1 or $weapon2 urgently required! I am willing to pay in $currency1."; }
	if ( $Ts == 14 ) { return "CONFIRMED: $person2 $verbpast1 $person1 using $thing1 and $thing2" . $ending . ""; }
	if ( $Ts == 15 ) { return "$begin1 $person1 is looking for $weapon1 or $weapon2 from $person2 $ending"; }
	if ( $Ts == 16 ) { return "Wanted: $weapon1 or $weapon2 urgently needed! Willing to pay in $currency1 or $currency2."; }
	if ( $Ts == 17 ) { return "Hello. The assasination of $person1 and $person2 is urgently needed. Willing to pay in $currency1 or $currency2..."; }
	if ( $Ts == 18 ) { return "Good day. I am looking for assistance assasinating $person1! We are willing to pay in $currency1 or $currency2."; }
	if ( $Ts == 19 ) { return "We need to dispose of $person1 using $weapon1, urgently! We are willing to pay in $currency1 or $currency2. Help!"; }
	if ( $Ts == 20 ) { return "$begin1 $person1 was found purchasing $weapon1 from $person2 paying with $currency1" . $ending . ""; }
	if ( $Ts == 21 ) { return "Looking for someone to destroy $location1 using $weapon1 or $weapon2. Will pay in $currency1 or $currency2!"; }
	if ( $Ts == 22 ) { return "Salaam my friends. Have you spoken with $person1 about aquiring $weapon1 or $weapon2? They might have them at $location1. I have also heard that $person2 may have them at $location2. Allah is good!"; }
	if ( $Ts == 23 ) { return "Allahu al akbar. I am seeking $weapon1, or if that is not available, $weapon2. $person1 told me that I could get them from $person2, but they wanted to be paid in $currency1."; }
	if ( $Ts == 24 ) { return "Good morning. I was told by $person1 that I could come here to obtain $weapon1. Is this true? I can pay in $currency1, although $person2 said that I could pay in $currency2. Thanks :)"; }
	if ( $Ts == 25 ) { return "Hey there. Does anyone know where I can obtain $weapon1? I could also use $weapon2. I am willing to pay in $currency1 or $currency2, $person1 referred me here. Thanks! Allahu al akbar."; }
	if ( $Ts == 26 ) { return "Hrm... $person1 said that I could come here and purchase $weapon1 with $currency1, or possibly $weapon2 with $currency2. Is this true? $person2 also mentioned it but I can't prove that, as they're on a business trip to $location1..."; }
	if ( $Ts == 27 ) { return "Sorry to interrupt, salaam my good friends... But do you know where I can purchase, using $currency1, $weapon1 or $weapon2? I am an FBI investigator, and I have records showing that $person1 was in talks with $person2 to aquire these items... Thanks."; }
	if ( $Ts == 28 ) { return "How are our plans going to destroy $location1 with $weapon1? $person1 tells me that all operations are a go to use $weapon2 to damage $location2, as well. All I need is the go ahead!"; }
	if ( $Ts == 29 ) { return "Hello my friends!! Allah's compassion be with you! I need to buy $weapon1 and $weapon2, I can pay in $currency1. Msg me with any info!"; }
	if ( $Ts == 30 ) { return "Loldongz hy. $person1 said that I could come here to purchase $weapon1 and/or $weapon2, using $person2 as a middleman. I can pay in $currency1 which was specified at the time. Hello?"; }
	if ( $Ts == 31 ) { return "I am in a situation. I have possession of large ammounts of $weapon1, and very large ammounts of $weapon2! I don't know what to do with these fuckin things... Someone buy them off of me, I will make it worth your time. I am willing to accept $currency1 or $currency2. We will use $person1 as a middleman, to increase security. Thanks for listening."; }
	if ( $Ts == 32 ) { return "When I was in the Black Projects, I aquired a large amount of $weapon1 and $weapon2. I've hidden them at $location1 and $location2, but now I want to get rid of them. If you are willing to pay in $currency1, please contact me. Allahu al akbar!"; }
	if ( $Ts == 33 ) { return "The other part of the story is that $person1 was able to secure me an extraordinary amount of $weapon1 and $weapon2. They had to be shipped via freighter to $location1. I paid with $currency1."; }
	if ( $Ts == 34 ) { return "Allahu al akbar. FOR SALE: fifteen crates of $weapon1 and nine crates of $weapon2. These were stolen from $location1 so I need to move these fast! I will only accept $currency1 and $currency2, we *verify* first!"; }
	if ( $Ts == 35 ) { return "Our friend $person1 will complete his mission, God willing: Destroy $location1 and $location2 in a spectacular attack, using $weapon1. Pray for our friend, praise Allah!"; }
	if ( $Ts == 36 ) { return "NEWS UPDATE for my friends! I have finally recommunicated with our Brother in Arms $person1. The very exciting news is that all units have aquired $weapon1 and $weapon2, and are now ready to rain Allah's fury upon $location1!! Pray for our friends!"; }
	if ( $Ts == 37 ) { return "Good morning my Brothers! After days of nothing, I recieved a communication from our friend $person1. It came in an encrypted ICMP packet payload. It reads as follows: prs allh! mv to attack $location1 with $weapon1 and also $location2. use $weapon2 as bckup, hwver I hope we not need it.	Status: CONFIRMED"; }
	if ( $Ts == 38 ) { return "Hello my brother! I am happy to report that with the help of $person1 and $person2, we have used $weapon1 to attack $location1. We tried to attack $location2 using $weapon2, however, our activities were uncovered! Our primary activities are complete."; }
	if ( $Ts == 39 ) { return "URGENT: We have uncovered the plans set forth by $person1. They were planning to attack $location1, possibly with $weapon1. We foiled their plans by purchasing the weapons using $currency1. We are still watching $person2 who recently aquired $weapon2, we are very worried about this developing situation!"; }
	if ( $Ts == 40 ) { return "I am here as per instructions of $person1. I have $currency1, all of it, as agreed, to purchase $weapon1. Do not try any funny tricks, we have $person2 on our side. Please hurry!"; }
	if ( $Ts == 41 ) { return "$person1 is recruiting Brothers in Arms for his plan to attack $location1! If you are interested, you MUST act quickly. Please contact $person2. We are willing to pay in $currency1, however, donations are much appreciated! PRAISE ALLAH!!"; }
	if ( $Ts == 42 ) { return "We are running low on resources! We desperately need $weapon1 and $weapon2. We can pay in $currency1, however, we may not be able to get it to you all at once. Please contact $person1 IMMEDIATELY if you can assist us in our Holy War!!"; }
	if ( $Ts == 43 ) { return "The capitalist infidels are a FIERCE enemy, we are having great difficulty contacting cell number $cellnum. $person1 claims that we are only weeks from being defeated. We must use our final plan to destroy $location1 and $location2 with $weapon1 !! THOSE ARE YOUR ORDERS MY FRIEND!"; }
	if ( $Ts == 44 ) { return "Have you received the shipment of $weapon1 and $weapon2 from $person2? I have already transferred the $money to your bank account."; }
	if ( $Ts == 45 ) { return "$weapon1 should arrive at $location1 shortly. $person2 is there to pick it up. I've already transferred the $barter_items to " . $person1 . "'s storage warehouse"; }
	if ( $Ts == 46 ) { return "We have uncovered " . $person1 . "'s plans to attack $location1 and $location2. It has also come to light that $person2 was funding $person1 with $currency1 for these attacks" . $ending . ""; }
	if ( $Ts == 47 ) { return "Does anyone have discounted $weapon1 for sale? I can't afford to be spending anymore $currency1 on stuff like $weapon2 and $thing1"; }
	if ( $Ts == 48 ) { return "That's wierd... http://www.cnn.com/ talks about how $person1 is threatening $location1 with $weapon1! quick go look!!"; }
	# Remember, read the comments above comment 0 if you add any structures
	#	if ($Ts == 49) { return "whatever"; }
	#	if ($Ts == 50) { return "whatever"; }
}